class sensucustom::emc ( 
  ...
  exec {'emc_security_file':
    command => "su -c '/opt/Navisphere/bin/naviseccli -secfilepath ${destination}/check_emc_clariion_security_files -User ${username} -Password ${password} -Scope ${scope} -AddUserSecurity' sensu",
